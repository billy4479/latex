...
\subsection{Divergence of \texorpdfstring{$\vec E$}{the electric field}}
...
first maxwell law or gauss law in local form.
...
\subsection{Curl of \texorpdfstring{$\vec E$}{the electric field}}

\begin{equation}
    \int_A^B \vec E \cdot \dd{l} = \int_A^B E \cos \theta \dd{l} = \int_A^B E \dd{r} = \frac{q}{4 \pi \varepsilon_0} \int_A^B \frac{1}{r^2} \dd{r} = \frac{q}{4 \pi \varepsilon_0} \left(\oneover{M_A} - \oneover{M_b}\right)
\end{equation}

if path is closed work is zero, E is conservative, by stokes theorem the curl is 0.
