...
\tikzset{>=latex}
\usetikzlibrary{calc,quotes,angles,snakes}
...
\begin{equation}
    \vec {B'} = \curl \vec{A '} = \curl \vec A + \underbrace{\curl (\grad f)}_{= 0} = \vec B
\end{equation}
this gives a great deal of freedom which can be used to remove the extra term in the partial differential equation above.
