...
Recall that a \emph{matching} $M$ is a subset of $E$ so that
\begin{equation}
...
Given a graph $G = (V, E)$ recall that a vertex cover is a set $C \subseteq V$ so that each edge in
$E$ has at least one of its vertices in $C$. Formally
\begin{equation}
	C = \{ v \mid v \in V \land ((u, w) \in E \implies (u = v \lor w = v)) \}
\end{equation}
...
\begin{proof}
	...
	Then $C = \{ v \in V \mid y^*_v = 1 \}$ is a vertex cover (since $A^T \vec y^* \geq \vec 1$) and
	$\abs{C} = \vec 1^T \vec y^*$.
...
\subsubsection{Directed graphs and flows}

\paragraph{Incidence matrix}
Let $G = (V, E)$ be a directed graph. The \emph{incidence matrix} $M$ is defined as
\begin{equation}
	M_{ve} = \begin{cases}
		+1 & \text{if $e$ leaves $v$} \\
		-1 & \text{if $e$ enters $v$} \\
...
\begin{theorem}{Incidence of directed graph is TU}{incidence-tu}
...
\begin{equation}
	...
	\end{cases}
\end{equation}
This is the problem for the minimum cut.
...
$\vec c$ and a cost vector $\vec \kappa$, we want to find the flow $\vec x$ of size $\alpha$ that
minimizes $\vec \kappa^T \vec x$.
...
Then $P' = \{ x \mid x \in P, \vec w^T \vec x = \alpha \}$ is a face of $P$ and therefore it is also
an integer polyhedron.
We have that $\forall \vec c \in \R^n$, $\argmax \{ \vec c^T \vec x \mid x \in P'\}$ is a face of
$P'$, therefore it contains an integer vector.
...
\begin{theorem}{Max-flow minimum-cut}{max-flow-min-cut}
