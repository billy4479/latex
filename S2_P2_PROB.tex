    ...
    \begin{equation}
        \begin{split}
            E\left( \left(\sum_{k = 1}^n Y_k\right)^4 \right) & = E\left( (Y_1 + \dots + Y_n)(Y_1 + \dots + Y_n)(Y_1 + \dots + Y_n)(Y_1 + \dots + Y_n) \right) \\
                                                              & = \sum_{i, j, k , e} E(Y_i Y_j Y_k Y_e)
        \end{split}
    \end{equation}
    and we can do some sorcery (TODO: read the book) to get that
    \begin{equation}
        \sum_{i, j, k , e} E(Y_i Y_j Y_k Y_e) = nm_4 + 3n(n-1)m_2^2
    \end{equation}
    where $m_k$ is the $k$-th moment of $Y_k$.
    But then
    \begin{equation}
        \sum_{n = 1}^\infty \frac{1}{n^4 \varepsilon^4}(nm_4 + 3 n(n-1)m_2^2) \sim \frac{1}{n^2} \implies \sum < +\infty
    \end{equation}
\end{proof}

\begin{proposition}
    Let $X_n$ i.i.d. with finite expectation and variance.
