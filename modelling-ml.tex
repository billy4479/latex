...
To do this we use gradient descent (how original!), therefore we need to compute the derivatives
w.r.t. the parameters $\theta = (\vec w, \vec b)$.
\begin{equation}
	\pdv{C}{\theta} = \frac{1}{M} \sum^M_{\mu = 1}
	\E\left[ \pdv{E(x^\mu, h)}{\theta} \right] - \E\left[ \pdv{E(x, h)}{\theta} \right]
\end{equation}
where we are taking the mean over $S$ using the Boltzmann distribution.

In practice, it is impossible to average over all neurons, therefore we use techniques like
mini-batch (cf. \cref{sec:mini-batch}) or contrastive divergence $k$ algorithm (??).
