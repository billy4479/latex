...
While training neural networks we want to minimize some kind of loss function
\begin{equation}
	L(\vec w) = \sum_{x \in X} \ell(\vec w, x)
\end{equation}
but in order to compute its gradient we will have to compute the gradient of all the points in the
dataset, which, depending on the size of the dataset, could be unfeasible.

In general, assume we have
\begin{equation}
	f(x) = \frac{1}{n} \sum_{i = 1}^{n} f_i(x)
\end{equation}
and stochastic gradient descent comes to our help. We sample $i \in \{ 1, \dots, n\}$ uniformly
at random and let $x_{t+1} = x_t - \gamma_t \grad f_i(x_t)$.

We let $g_t = \grad f_i(x_t)$, which will be the gradient used at iteration $t$.
We note that $g_t$ is an unbiased estimator of $\grad f(x_t)$ given $x_t$:
\begin{equation}
	\E[g_t] = \sum_{i = 1}^{n} \frac{1}{n} \grad f_i (x_t) = \grad f(x_t)
\end{equation}

In particular, we'd like to compute $\E[g_t^T (x_t - x^*)]$. Conditioning on $x_t = x$ we get that
\begin{equation}
	\E[g_t^T (x_t - x^*) \mid x_t = x] = \E[g_t \mid x_t = x]^T (x - x^*) = \grad f(x)^T(x - x^*)
\end{equation}
We now want to remove the conditioning on $x_t = x$. Since $X$ is finite, $x_t = x$ can only occur a
finite amount of times, therefore
\begin{align}
	\E(g_t \mid x_t = x) & = \sum_{x \in X}  \E[g_t^T (x_t - x^*) \mid x_t = x]  P(x_t = x) \\
	                     & = \sum_{x \in X} \grad f(x)^T(x - x^*)  P(x_t = x)               \\
	                     & = \E[\grad f(x_t)^T(x_t-x^*)]
                       & \geq \E[f(x_t) - f(x^*)]
\end{align}
by convexity of $f$.

\begin{theorem}{Stochastic gradient descent}{stochastic-gradient-descent}
  Let $f: \R^d \to \R$ be a convex differentiable function and $x^*$ one of its global minimums.
  Let $\norm{x_0 - x^*} \leq R$ and $\E[\norm{g_t}^2] \leq B^2$.
  With learning rate $\gamma = \frac{R}{B \sqrt{T}}$ we have
  \begin{equation}
    \frac{1}{T} \sum_{t = 0}^{T - 1} \E[f(x_t)] - f(x^*) \leq \frac{RB}{\sqrt{T}}
  \end{equation}
\end{theorem}
\begin{proof}
  TODO: basically vanilla analysis, maybe nice exercise?
\end{proof}
