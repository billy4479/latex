...
\begin{theorem}{Minkowski-Weyl for polytopes}{minkowski-weyl}
	$P \subseteq \R$ is a polytope if and only if $\exists V \subseteq \R^n$ finite such that
	$P = \mathrm{conv}(V)$ (convex hull of $V$).
\end{theorem}

\begin{theorem}{Minkowski-Weyl for polyhedra}{minkowski-weyl}
...
\begin{lemma}{Equivalent definitions of faces}{faces}
	...
	\begin{enumerate}
		\item The definition we used in \cref{def:face}:
		      \begin{equation}
...
\begin{lemma}{Dimension of proper faces}{dim-faces}
	...
	\begin{itemize}
		\item $\mathrm{dim}(P) = n - \mathrm{rank}(A')$;
...
\begin{proof}
	...
	$\dim(P) \leq \dim(A) = n - \mathrm{rank}(A)$.
...
\begin{proof}
	...
	Then $\dim(F) \leq n - \mathrm{rank}\begin{pmatrix} A' \\ \tilde A \end{pmatrix}$.

	If $\mathrm{rank}\begin{pmatrix} A' \\ \tilde A \end{pmatrix} = \mathrm{rank}(A')$ then
	either each inequality of $\tilde A x = \tilde b$ is a linear combination of $A' x = b'$
	and $F = P$, otherwise we have
	\begin{equation}
		\mathrm{rank} \begin{pmatrix} A' & b' \\ \tilde A & \tilde b \end{pmatrix} >
		\mathrm{rank} \begin{pmatrix} A' \\ \tilde A \end{pmatrix} = \mathrm{rank}(A')
	\end{equation}
	which implies $F = \varnothing$.

	Otherwise we have that
	$\mathrm{rank} \begin{pmatrix} A' \\ \tilde A \end{pmatrix} \geq \mathrm{rank}(A') + 1$
	and
	\begin{equation}
		\dim(F) \leq n - \mathrm{rank}(A') - 1 = \dim(P) -1 \qedhere
	\end{equation}
\end{proof}
...
\begin{proof}
	...
	\begin{equation}
		\dim(F) = n - \mathrm{rank}\begin{pmatrix} A'\\a^T \end{pmatrix} = n- \mathrm{rank}(A') + 1
	\end{equation}
...
\begin{theorem}{Dimension of minimal faces}{dim-minimal-faces}
	All minimal faces of $P$ have dimension $n - \mathrm{rank}\begin{pmatrix} A' \\ A'' \end{pmatrix}$.
\end{theorem}
...
\begin{proof}
	...
	Then
	\begin{equation}
		\dim(F) = n- \mathrm{rank}\begin{pmatrix} A' \\ \tilde A \end{pmatrix}
		\geq n - \mathrm{rank}\begin{pmatrix} A' \\ A''\end{pmatrix}
	\end{equation}
	We claim that they are equal.
	By contradiction say $\mathrm{rank} \begin{pmatrix} A' \\ \tilde A \end{pmatrix} \leq
		\mathrm{rank} \begin{pmatrix} A' \\ A'' \end{pmatrix}$.
