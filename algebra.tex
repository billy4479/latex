...
\begin{theorem}{Definition of Orthonormal Basis}
    ...
    As a corollary of the previous theorem we have that \textbf{every orthonormal list of vectors is linearly indipendent}.
...
\begin{theorem}{Riesz Representation Theorem}
    ...
    Let $V$ be finete-dimensional and $\varphi: V \to \F$.
...
\begin{theorem}{Eigenvalues of Self-Adjoint Mapping are Real}
    ...
    then $P_A$ (characteristical polynomial) has all roots in $\R$.
\end{theorem}
...
\begin{theorem}{The Spectral Theorem}
    ...
    Then there exists an orthonormal basis of $V$ consisting of eigevectors of $T$.

    Moreover if $\F = \R$ this is an equivalence (if and only if).

    \begin{proof}
        Let $\lambda$ be an eigenvalue of $T$ and $u$ its eigenvalue.
