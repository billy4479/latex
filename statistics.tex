...
\begin{example}{Exponential distribution}{mle-exponential}
    ...
    We conclude that $\hat \lambda = \frac{1}{\overline{x}}$ is the maximum likelihood estimate.
\end{example}
...
\begin{example}{Coin tosses}{mle-bernoulli}
    ...
    \begin{align}
        L(p; X_1, \dots, X_n)         & = \prod_{i = 1}^n \left( p^{X_i} (1 - p)^{1-X_i} \right)   \\
                                      ...
                                      & = p^{n \overline X} (1-p)^{n(1-\overline{X})}              \\
        l(p; X_1, \dots, X_n)         & = n \overline X \log p + n(1- \overline{X}) \log(1-p)      \\
        \dv{l(p; X_1, \dots, X_n)}{p} & = \frac{n \overline x}{p} - \frac{n(1-\overline{x})}{1-p}
    \end{align}

    Now we have our score function that we want to maximize:
    \begin{align}
        \max_p                           & \frac{n \overline x}{p} - \frac{n(1-\overline{X})}{1-p}                             \\
                                         & \implies \frac{\cancel {n} \overline X}{p} = \frac{\cancel{n}(1-\overline{X})}{1-p} \\
                                         & \implies \overline{X} (1-p) = p (1-\overline X)                                     \\
                                         & \implies \overline{X} -\cancel{p\overline{X}} = p -\cancel{p\overline{X}}
        \implies \hat p = \overline{X}                                                                                         \\
        \dv[2]{l(p; X_1, \dots, X_n)}{p} & = -\frac{n \overline{X}}{p^2} - \frac{n(1-\overline X)}{(1-p)^2} < 0                \\
...
\begin{example}{Normal distribution}{mle-normal}
    ...
    \begin{align}
        \sum_{i = 0}^{n} \frac{(X_i - \hat \mu)}{\sigma^2} = 0                      & \implies \hat \mu = \overline X                                            \\
        -\frac{n}{2 \sigma^2} + \sum_{i = 1}^n \frac{(X_i - \mu)^2}{2 \sigma^4} = 0 & \implies \hat{\sigma}^2 = \frac{1}{n} \sum_{i = 1}^n (X_1 - \overline X)^2
    \end{align}
...
\begin{definition}{Moment and sample moment}{moment-sample-moment}
    ...
    \begin{equation}
        \overline{X_j} = \frac{1}{n} \sum_{i = 1}^n X_i^j \convas E_\theta(X^j)
    \end{equation}
...
\begin{example}{Common distributions}{}
    \begin{itemize}
        ...
        \item \emph{Uniform}: $\hat \theta = 2 \overline X$ which is a worse result than the MLE.
    \end{itemize}
\end{example}
