...
\end{align}
Note that this only works if we can \say{take out} the expectation from the sum, but indeed we will be able to do so in some cases.
...
\begin{proof}
    ...
    First, we will compute $Z_n$ explicitly. We have
    \begin{align}
    ...
    then
    \begin{equation}
        Z_n = \frac{S_n - E(S_n)}{\sqrt{V(S_n)}}= \frac{S_n - n \mu}{\sigma \sqrt{n}} = \frac{1}{\sqrt {n}} \frac{\sum_{i = 0}^n X_i - n \mu}{\sigma} = \frac{1}{\sqrt{n}} \sum_{i = 0}^n \underbrace{\frac{X_i - \mu}{\sigma}}_{Y_i} = \sum_{i = 0}^n \frac{Y_i}{\sqrt{n}}
    \end{equation}
    where $Y_i$ is the normalization of $X_i$.
    Note that $Y_i$ are still independent and identically distributed.

    We now want to show that the mgf of $Z_n$ is the one of a normal distribution.
    Start by considering the taylor expansion of the mgf up to the second term for $t \to 0$
    \begin{equation}
        M_X(t) = 1+E(X) t + \frac{E(X^2) t^2}{2} + \o(t^2)
    \end{equation}
    and fix $t$ and compute $M(t/ \sqrt{n})$ such that as $n \to \infty$, $t/ \sqrt{n} \to 0$:
    \begin{equation}
        M_X\left(\frac{t}{\sqrt{n}}\right) = 1+E(X) \frac{t}{\sqrt{n}} + \frac{E(X^2) t^2}{2n} + \o\left(\frac{1}{n}\right)
    \end{equation}

    We now calculate the moment generating function of $Z_n = \sum_{i = 0}^n \frac{Y_i}{\sqrt{n}}$: we have
    \begin{equation}
    ...
    \end{equation}
    which means that the mgf of $Y_n$ is well defined in a symmetric interval around 0, implying we can use \autoref{prop:mgf:props_of_mgf} to write the mgf of $Z_n$ as
    \begin{equation}
        M_{Z_n}(t) = \prod_{i = 1}^n M_{Y_i}\left(\frac{t}{\sqrt{n}}\right) = \left(M_{Y_1}\left(\frac{t}{\sqrt{n}}\right)\right)^n
    \end{equation}
    since $Y_i$ are i.i.d..
...
\begin{theorem}[Markuo inequality]
...
\begin{definition}[convergence in the $r$-th mean]
    Let $r \geq 1$.
...
\begin{definition}[law of large number]
...
\begin{theorem}[weak law of large numbers for uncorrelated r.v.]
    \label{thm:conv:wlln}
...
\begin{theorem}[strong law of large numbers for i.i.d. r.v.]
