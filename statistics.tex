...
\begin{itemize}
    \item The \emph{Bayes criterion} (\Cref{sec:bayes}) minimizes the average MSE.
    \item The \emph{Minmax criterion} minimizes the worst case scenario MSE:
          \begin{equation}
              \sup_{\theta \in \Theta} MSE(T(x), g(\theta))
          \end{equation}
    \item \emph{Universal Minimum Variance Uniform estimator} (UMVU) is an estimator for $g(\theta)$
