...
\begin{itemize}
    ...
    \item An \textbf{all-black} path from $s$ to $v$ is a path where each node, except possibly $v$, is black.
\end{itemize}
...
\begin{proof}
    The proof is based on induction, similar to the proof for BFS.
    We will find two invariants that will help us prove the statement.

    We will need to prove several lemmas:
    \begin{description}
        \item[Lemma 1] At the end of each while loop
            \begin{enumerate}[label=\roman*]
                ...
                \item All nodes $v$ have $d(v)$ equal to the length of the shortest \textit{all-black} path from $s$ to $v$.
            \end{enumerate}

            Suppose that both statements are true at time $t$.
            Let $v$ be the vertex that is removed from the queue during the $t+1$-th iteration.

            We now want to prove that (i) and (ii) at time $t$ $\implies$ (i) at time $t+1$.

            Note that the distance $d(v)$ remains what it was at time $t$.
            That is $d_{t+1}(v) = d_t(v)$.

            Suppose that at time $t+1$ there is a path of all black vertices from $s$ to $v$ of length $L < d_{t+1}(v)$.
            But, since $d_{t+1}(v) = d_t(v)$ and (ii) was true at $t$, (ii) implies that there must be a white vertex along such path, let $u$ be such white vertex.
            The length of this path 

            TODO: look at the pdf about this
