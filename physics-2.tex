...
In \Cref{fig:hysteresis} we can see how the free magnetic field $\mu_0 H$ influences the total magnetic field $B$.
...
as we can see in \Cref{fig:ising}.
...
\begin{figure}[H]
    ...
    \subfloat[Hysteresis loop: $\mu_0 H$ represents the external field, while $B$ is the field we actually get.
        As you can see $B$ is $10^4$ orders of magnitude greater. \label{fig:hysteresis}]{\begin{tikzpicture}
        ...
        \end{tikzpicture}}
    \qquad
    \subfloat[At Curie temperature $T_C$ ferromagnetic materials become paramagnets. \label{fig:ising}]{\begin{tikzpicture}
            \begin{axis}[very thick,
                    samples = 25,
                    xlabel = $T$,
                    ylabel = $M$,
...
\begin{example}{}{}
    Consider an infinitely long cylinder centered around the $z$ axis
    with uniform \emph{frozan} magnetization $\vec M \hat z$.
    Find $\vec B$.
\end{example}
...
\begin{proof}[Solution]
    ...
    \begin{figure}[H]
        \centering
        \includesvg[width=0.3\textwidth]{assets/physics-2/ex-ferromagnetic-cylinder.svg}
...
the induced current will go \say{against} the change of flux
by generating a $\vec B_\text{ind}$ in the opposite direction of the change of flux.

\subsubsection{First experiment}
Consider the system in \Cref{fig:faraday-exp-1}.

\begin{figure}[H]
    \centering
    \includesvg[width=0.5\textwidth]{assets/physics-2/faraday-exp1.svg}
...
\begin{figure}[H]
    \centering
    \includesvg[width=0.3\textwidth]{assets/physics-2/faraday-exp1-velocities.svg}
...
\begin{figure}[H]
    \centering
    \includesvg[width=0.5\textwidth]{assets/physics-2/faraday-exp1-general.svg}
...
\begin{example}{RL circuit}{rl-circuit}
    ...
    \begin{figure}[H]
        ...
        \begin{circuitikz}
            ...
            % Draw the closing switch
            \draw (2,2) to[closing switch] (4,2);

            % Draw the inductor on the right segment
