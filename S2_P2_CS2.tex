...
Furthermore there is a subset of $\NP$ of problems called $\NP$-complete: it is possible to prove that if someone were to find an efficient solution to any of these problems they would almost automatically find an efficient solution for all of them.
...
When dealing with our own problem $X$ and we don't want to solve it we can either show it reduces to a problem in $P$ or, if we cannot find an efficient algorithm for it, we can reduce an $\NP$-complete problem $Y$ to $X$ showing that $X$ itself is $\NP$-complete.
...
\subsection{Main theorem of NP-completeness}
